...
class BottleneckX(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, dilation=1):
        ...
        bottle_planes = dim * BottleneckX.cardinality
...
class DLA(nn.Module):
    def __init__(self, levels, channels, num_classes=1000,
                 block=BasicBlock, residual_root=False, return_levels=False, return_levels_list=[],
                 pool_size=7, linear_root=False):
        super(DLA, self).__init__()
        self.channels = channels
        self.return_levels = return_levels
        self.return_levels_list = return_levels_list
    ...
    def forward(self, x):
        ...
        for i in range(6):
            ...
            if self.return_levels_list == []:
                y.append(x)
            else:
                if i in self.return_levels_list:
                    y.append(x)
...
def dla34_fpn(pretrained=None, **kwargs):
    model = DLA([1, 1, 1, 2, 2, 1],
                [16, 32, 64, 128, 256, 512],
                block=BasicBlock, return_levels=True, return_levels_list=[2, 3, 4, 5], **kwargs)
    # if pretrained is not None:
    #     model.load_state_dict(torch.load(pretrained), False)
    model.load_state_dict(torch.load('/home/niejiadong/dla_pretrain_model/dla34-ba72cf86.pth'), False)
    return model
...
if __name__ == '__main__':
    ...
    m = dla34_fpn(pretrained='/home/niejiadong/dla_pretrain_model/dla34-ba72cf86.pth')
    # m = dla46_c()
    # checkpoint = torch.load('/home/niejiadong/dla_pretrain_model/dla46_c-2bfd52c3.pth')
    # m.load_state_dict(checkpoint, False)

...
try:
    ...
    from maskrcnn_benchmark.modeling.backbone.efficientnet_utils import (
        relu_fn,
        round_filters,
        round_repeats,
        drop_connect,
...
class EfficientNet(nn.Module):
    ...
    def __init__(self, blocks_args=None, global_params=None, feature_only=True):
        super().__init__()
        ...
        # distribute training failed when define the not used layers and weights
        if not self._feature_only:
            # Head
    ...
    def extract_features(self, inputs):
        ...
        for idx, block in enumerate(self._blocks):
            cur_stride = block._block_args.stride
            if isinstance(cur_stride, list):
                cur_stride = cur_stride[0]
            if cur_stride > 1:
                y.append(x)
    ...
    @classmethod
    def from_name(cls, model_name, override_params=None, feature_only=True):
        cls._check_model_name_is_valid(model_name)
        blocks_args, global_params = get_model_params(model_name, override_params)
        return EfficientNet(blocks_args, global_params, feature_only)

    @classmethod
    def from_pretrained(cls, model_name, feature_only=True):
        model = EfficientNet.from_name(model_name, feature_only=feature_only)
        load_pretrained_weights(model, model_name)
        return model

    @classmethod
    def efficientnet_b3_fpn(cls):
        return EfficientNet.from_pretrained('efficientnet-b3', feature_only=True)
...
if __name__ == '__main__':
    ...
    # with SummaryWriter(logdir='/home/niejiadong/log') as w:
    #     w.add_graph(m, (a,))

    # for name, param in m.named_parameters():
    #     print(name, True if param.grad is not None else False)

    # torch.onnx._export(m,
    #                    a,
    #                    "frozen.onnx",
    #                    export_params=True
    #                    )

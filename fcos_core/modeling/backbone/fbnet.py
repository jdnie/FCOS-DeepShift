...
class FBNetTrunk(nn.Module):
    def __init__(
        self, builder, arch_def, dim_in, need_fpn=False
    ):
        super(FBNetTrunk, self).__init__()
        self.first = builder.add_first(arch_def["first"], dim_in=dim_in)
        trunk_cfg = _get_trunk_cfg(arch_def)
        self.stages = builder.add_blocks(trunk_cfg["stages"], need_fpn)
        if need_fpn:
            for i, s in enumerate(self.stages):
                self.add_module("stage{}".format(i), s)
...
@registry.BACKBONES.register("FBNet-FPN")
def add_conv_body_fpn(cfg, dim_in=3):
    builder, arch_def = create_builder(cfg)

    body = FBNetTrunk(builder, arch_def, dim_in, need_fpn=True)
    model = nn.Sequential(OrderedDict([("body", body)]))
    model.out_channels = builder.last_depth
...
if __name__ == '__main__':
    ...
    a = torch.rand(16, 3, 640, 640)

    cfg.MODEL.FBNET.ARCH = "cham_v1a"
    cfg.MODEL.FBNET.BN_TYPE = "bn"
    cfg.MODEL.FBNET.WIDTH_DIVISOR = 8
    cfg.MODEL.FBNET.DW_CONV_SKIP_BN = True
    cfg.MODEL.FBNET.DW_CONV_SKIP_RELU = True
    m = add_conv_body_fpn(cfg)

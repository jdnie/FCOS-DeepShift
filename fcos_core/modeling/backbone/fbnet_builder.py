...
class FBNetBuilder(object):
    ...
    def add_blocks(self, blocks, need_fpn=False):
        """ blocks: [{}, {}, ...]
        """
        ...
        for block in blocks:
            ...
            if need_fpn:
                if str(stage_idx) not in modules:
                    modules[str(stage_idx)] = OrderedDict()
                # 同一个block不能同时再次出现
                assert nn_name not in modules[str(stage_idx)]
                modules[str(stage_idx)][nn_name] = nnblock
            else:
                assert nn_name not in modules
                modules[nn_name] = nnblock
        if need_fpn:
            ret = [nn.Sequential(modules[m]) for m in modules]
        else:

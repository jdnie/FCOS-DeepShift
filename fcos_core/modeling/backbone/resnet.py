...
from fcos_core.modeling.backbone.downsample import *
...
# ResNet-22-FPN (including all stages)
ResNet22FPNStagesTo5 = tuple(
    StageSpec(index=i, block_count=c, return_features=r)
    ...
    def __init__(
        ...
        if in_channels != out_channels:
            ...
            if down_stride > 1 and self.aa:
                self.downsample = nn.Sequential(
                    OrderedDict([
                        ('aa', Downsample(filt_size=3, stride=stride, channels=in_channels)),
                        ('conv', Conv2d(
                            in_channels, out_channels,
        ...
        if stride_1x1 > 1 and self.aa:
            self.conv1 = nn.Sequential(
                OrderedDict([
                    ('aa', Downsample(filt_size=2, stride=stride_1x1, channels=in_channels)),
                    ('conv', Conv2d(
                        in_channels,
                        bottleneck_channels,
                        kernel_size=1,
        ...
        if with_dcn:
            ...
            self.conv2 = nn.Sequential(
                OrderedDict([
                    ...
                    ('aa', Downsample(filt_size=3, stride=stride_3x3, channels=bottleneck_channels))
                ])
            )
...
class BaseStem(nn.Module):
    def __init__(self, cfg, norm_func):
        ...
        if cfg.MODEL.RESNETS.STEM_3X3X3:
            self.conv1 = nn.Sequential(
                Conv2d(
                    3, out_channels//2, kernel_size=3, stride=2, padding=1, bias=False
                ),
                FrozenBatchNorm2d(out_channels//2, True),
                Conv2d(
                    out_channels//2, out_channels//4, kernel_size=3, stride=1, padding=1, bias=False
                ),
                FrozenBatchNorm2d(out_channels//4, True),
                Conv2d(
                    out_channels//4, out_channels, kernel_size=3, stride=1, padding=1, bias=False
                ),
            )
        ...
        if self.no_maxpool:
            self.maxpool = nn.Sequential(
                Conv2d(
                    out_channels, out_channels//2, kernel_size=3, stride=1, padding=1, bias=False
                ),
                FrozenBatchNorm2d(out_channels//2, True),
                Conv2d(
                    out_channels//2, out_channels, kernel_size=3, stride=2, padding=1, bias=False
                ),
            )
...
_STAGE_SPECS = Registry({
    ...
    "R-22-FPN-RETINANET": ResNet22FPNStagesTo5,
})


if __name__=='__main__':
    x = torch.randn((1,3,224,224))
    resnet = ResNet50StagesTo5()
    y = resnet(x)
    print(y.size())